import pika
from .trading_system_basic import BreadtAccount, BreadtPosition, BreadtTradeContract, BreadtSignalDirect
...
class TradeExecutor:
    ...
    def buy(self, symbol, price, quanty, ptype):
        ...
        if symbol.index('688') == 0 and r < 200 and self.account.available_money > 200 * price:
            r = 200
    ...
    def sell(self, symbol, price, quanty, ptype):
        if symbol not in self.positions.names() or self.positions[symbol].available_quanty == 0:
        ...
        if symbol.index('688') == 0 and r < 200 and self.positions[symbol].available_quanty > 200:
            r = 200
    ...
    def order(self, symbol, direct, price, quanty, ptype):
        ...
        connector = RabbitMQConnector(self.configfile)
        connector.send_message(self.order_exchange, 'trader.{}'.format(self.name), json.dumps(o))
